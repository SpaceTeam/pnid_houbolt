...
*TODO: Consider changing checkbox, slider and textEntry to "input" and adding a "style" value defining which kind of input it should be*
...
*TODO: Consider adding something like "highTreshold" which would check for the actual raw value instead of the interpreted/formatted value and would be a threshold like eg: `0.5` where `x > 0.5 == true` and `x < 0.5 == false`. If this is needed it may need an additional value for defining "direction" so eg `x > 0.5 == false` instead of true.*
