...
* Action Reference: A reference that can bundle the popups for several elements into one and sends its reference to the LLServer. This is used for example for `fuel_press_depress` where the press solenoid and depress solenoid both have the action reference `gui:fuel_press_depress` and open the same popup that sends a state with the action referece as name to the LLServer which in turn updates the corresponding elements as needed.
