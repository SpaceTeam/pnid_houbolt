...
function create_html(schematic, lib, content_manipulator) {
    ...
    function createTextField(shape, x, y, orientationMatrix) { //TODO revert the transformation matrix so that the text orientation is correct
        ...
        var h = `        <text class="sensor-letter" x="${x + posX}" y="${y - posY}" dominant-baseline="middle" text-anchor="middle"`;
    ...
    function addComponent(comp) {
        ...
        html = `    <g id="${comp.Reference}" class="${comp.Component_Name.replace(":","-")} comp" `;
    ...
    function addText(comp, orientationMatrix, compX, compY) { // TODO: add all text elements and make sure that the orientation is correct, also maybe use input tags instead of text
        ...
        if (Number(f0.dunno2) == 0) {
            ...
            y = y - 2*(y-compY);
            if (/L/.test(f0.Horizontal_Position)) {
                html += `        <text class="reference" x="${(x)}" y="${(y)}" dominant-baseline="middle" text-anchor="start"`;
                html += createTextTransformMatrices(x,y,orientationMatrix);
		        html += `>${f0.content}</text>\n`;
            } else if(/R/.test(f0.Horizontal_Position)) {
                html += `        <text class="reference" x="${(x)}" y="${(y)}" dominant-baseline="middle" text-anchor="end"`;
                html += createTextTransformMatrices(x,y,orientationMatrix);
		        html += `>${f0.content}</text>\n`;
            } else {
                html += `        <text class="reference" x="${(x)}" y="${(y)}" dominant-baseline="middle" text-anchor="middle"`;
                html += createTextTransformMatrices(x,y,orientationMatrix);
		        html += `>${f0.content}</text>\n`;
            } 
        }
        ...
        if (Number(f1.dunno2) == 0) {
            ...
            if (/L/.test(f1.Horizontal_Position)) {
                html += `        <text class="value" x="${(x)}" y="${(y)}" dominant-baseline="middle" text-anchor="start"`;
                html += createTextTransformMatrices(x,y,orientationMatrix);
		        html += `>${f1.content}</text>\n`;
            } else if(/R/.test(f1.Horizontal_Position)) {
                html += `        <text class="value" x="${(x)}" y="${(y)}" dominant-baseline="middle" text-anchor="end"`;
                html += createTextTransformMatrices(x,y,orientationMatrix);
		        html += `>${f1.content}</text>\n`;
            } else {
                html += `        <text class="value" x="${(x)}" y="${(y)}" dominant-baseline="middle" text-anchor="middle"`;
                html += createTextTransformMatrices(x,y,orientationMatrix);
		        html += `>${f1.content}</text>\n`;
            } 
        }
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            for (var j = 0; j < groups.length; j++) {
                for (var k = 0; k < groups[j].length; k++) {
                    if (groups[j][k][0] == arr[i][0] && groups[j][k][1] == arr[i][1] && groups[j][k][2] == arr[i][2] && groups[j][k][3] == arr[i][3]){
                        iInGroup = true;
                        currentGroupIndex = j;
                        break;
                    }
            ...
            if (!iInGroup) {
                ...
                groupsChanged = true;
                groups[currentGroupIndex] = [[arr[i][0], arr[i][1], arr[i][2], arr[i][3]]];
            }
            
            
            for (var j = i + 1; j < arr.length; j++) {
                var line = arr[i];
                if ((line[0] == arr[j][0] && line[1] == arr[j][1]) || (line[0] == arr[j][2] && line[1] == arr[j][3]) ||
                (line[2] == arr[j][0] && line[3] == arr[j][1]) || (line[2] == arr[j][2] && line[3] == arr[j][3]) &&
                !(line[0] == arr[j][0] && line[1] == arr[j][1] && line[2] == arr[j][2] && line[3] == arr[j][3])) {
                    groupsChanged = true;
                    var element = groups[currentGroupIndex];
                    element = element.concat([[arr[j][0], arr[j][1], arr[j][2], arr[j][3]]]);
                    groups[currentGroupIndex] = element;
                }
            }
        }
        return groups;
