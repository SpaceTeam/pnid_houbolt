...
function create_html(schematic, lib, content_manipulator) {
    ...
    function createTextField(shape, x, y) { //TODO revert the transformation matrix so that the text orientation is correct
        [posX, posY] = convertCoordinates(shape["posX"], shape["posY"]);
        var h = `        <text x="${Math.round(x + posX)}" y="${Math.round(y - posY)}" dominant-baseline="middle" text-anchor="middle">${shape["text"]}<text>\n`;
        return h;
    }
    ...
    function addComponent(comp) {
        ...
        html = `    <g id="${comp.Reference}" stroke="black" fill="none" `;
            ...
            if (key == compName) {
                for (var i = 0; i < lib[key].length; i++) {
                    ...
                    if (shape["type"] == "polygon") {
                        ...
                        text = createTextField(shape, x, y);
                    }
                }
            }
        }
    ...
    '<html>\n' + '<head>\n' + '    <title>PNID Chart</title>\n' + '</head>\n' + '<body>\n' + `<svg width="${screenX + screenX/10}" height="${screenY + screenY}">\n`;
