...
function create_html(schematic, lib, exportFilePath, content_manipulator) {
    ...
    function addComponent(comp) {
		...
		var compActionReference = compValReference;
		if (comp.Fields.Action_Reference != undefined)
		{
		    if (comp.Fields.Action_Reference.content !== "")
		    {
        ...
        svg = `    <g class="${comp.Reference} ${comp.Component_Name.replace(":","-")} ${compValReference} comp" data-value="40" data-unit="${unit}" data-${comp.Component_Name.replace(":","-").toLowerCase()}="" action-reference="${compActionReference}" pointer-events="bounding-box" onclick="clickEventListener('${compActionReference}')"`;
