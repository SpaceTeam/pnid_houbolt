...
function create_html(schematic, lib, exportFilePath, content_manipulator) {
    ...
    function addComponent(comp) {
        ...
        svg = `    <g class="${comp.Reference} ${comp.Component_Name.replace(":","-")} ${compValReference} comp" data-value="40" data-unit="${unit}" data-${comp.Component_Name.replace(":","-").toLowerCase()}="" action-reference="${compActionReference}" pointer-events="bounding-box" onclick="clickEventListener('${clickListener}')"`;
    ...
    function addText(comp, orientationMatrix, compX, compY) { // TODO: add all text elements and make sure that the orientation is correct, also maybe use input tags instead of text
        ...
        for (let prop in comp.Fields) {
            if (comp.Fields.hasOwnProperty(prop) && !(prop === "Reference" || prop === "Value" || prop === "Footprint" || prop === "Datasheet" || prop === "Action_Reference")) {
                f = comp.Fields[prop];
    ...
    var svg = '<svg style="position: relative; top: 0; left: 0; max-width: 100%; max-height: calc(100vh - 150px);" viewBox="450 500 10800 6100">\n';
