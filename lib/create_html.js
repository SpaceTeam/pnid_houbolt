...
function create_html(schematic, lib, exportFilePath, content_manipulator) {
    ...
    function addComponent(comp) {
		...
		var compValReference = comp.Fields.Value.content.replaceAll(":","-");
		...
		if (comp.Fields.Action_Reference != undefined)
		{
		    ...
		    {
		        compActionReference = comp.Fields.Action_Reference.content.replaceAll(":","-");
		    }
		}
		var compDataContent = "";
        if (comp.Fields.Data_Content != undefined)
        {
            if (!(comp.Fields.Data_Content.content === "" || comp.Fields.Data_Content.content === " "))
            {
                ...
                compDataContent = comp.Fields.Data_Content.content.replaceAll(":","-");
            }
        }
		var clickListener = "";
        ...
        if (comp.Reference.includes("?"))
        {
            console.log("Warning on parsing PnID: Reference (", comp.Reference, ") of", comp.Component_Name, "contains a '?'. It will be removed, but this could lead to reference ambiguity!");
        }
        //I dislike having to filter out "short" and "slim" variants here
        svg = `    <g class="${comp.Reference.replaceAll("?", "")} ${comp.Component_Name.replaceAll(":","-").replace("_Slim", "").replace("_Short", "").replace("_Small", "")} 
        ${compValReference} comp" data-unit="${unit}" data-${comp.Component_Name.replaceAll(":","-").replace("_Slim", "").replace("_Short", "").replace("_Small", "").toLowerCase()}="" 
        data-content="${compDataContent}" data-action-reference="${compActionReference}" data-set-state="" data-value="" data-action-reference-value=""`;
    ...
    function addText(comp, orientationMatrix, compX, compY) { // TODO: add all text elements and make sure that the orientation is correct, also maybe use input tags instead of text
        ...
        if (comp.Fields.Action_Reference != undefined)
		{
		    if (!(comp.Fields.Action_Reference.content === "" || comp.Fields.Action_Reference.content === " "))
		    {
		        f2 = comp.Fields.Action_Reference.content.replaceAll(":","-");
		    }
		}
        var html="";
        var visibility;
        ...
        //todo: consider removing this as well, I don't think we need a separate value out for action ref value, this should be sufficiently handled by the normal value field I think
        html += createTextTransformMatrices(x,y,orientationMatrix);
        html += `>${f2}</text>\n`;
    ...
    var svg = '<svg style="position: relative; margin: auto; top: 0; left: 0; max-width: var(--pnid-max-width); max-height: var(--pnid-max-height);" viewBox="">\n'; //viewbox sizing will be entered later after figuring out the bounds
    ...
    schematic.Text.forEach(t => {
        if (t.type == "Label") {
            textLabels.push([Number(t.x), Number(t.y), t.value.replaceAll(":","-")]);
        }
    });
    ...
    let padding = 0.03; //padding of content size from 0 to 1 (0 to full screen Width or so please correct)
