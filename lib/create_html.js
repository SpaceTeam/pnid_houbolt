...
function create_html(schematic, lib, content_manipulator) {
    ...
    function addComponent(comp) {
        ...
        html = `    <g class="${comp.Reference} ${comp.Component_Name.replace(":","-")} ${compValReference} comp" data-value="40" data-unit="${unit}" data-${comp.Component_Name.replace(":","-").toLowerCase()}="" pointer-events="bounding-box" onclick="clickEventListenever('${comp.Reference}', '${comp.Component_Name.replace(":","-")}', '${compValReference}')"`;
    ...
    '<html>\n' + '<head>\n' + '    <title>PNID Chart</title>\n' + `    <link rel="stylesheet" href="css/pnid.css">\n` +
    `    <link rel="stylesheet" href="css/popup.css">\n` +
	'    <link href="https://fonts.googleapis.com/css?family=Montserrat" rel="stylesheet">\n' +
    '    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>\n' + '</head>\n' +
    '<body>\n' +
	`<svg style="position: absolute; top=0; left=0;" viewBox="${minX - xBorder} ${minY - yBorder} ${maxX + xBorder} ${maxY + yBorder}">\n`;
    ...
    fs.writeFile("client/PnID_GroßerTeststand.html", html, function(err) {
        if(err) {
